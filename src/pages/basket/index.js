...
const Basket = ({ posts, favorites, basket, onFavorite, onBuy }) => {
  const onAdd = (id, type) => () => {
  ...
  const renderItems = () => {
    const arr = posts.filter((p) => basket.includes(p.id) && p);
...
export default connect(({ posts, favorites, basket }) => ({ posts, favorites, basket }), { onFavorite, onBuy })(Basket);
